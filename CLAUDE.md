...
**Current status:** Phase 1 — Orchestration Engine Hardening ✅ (instance cancellation, operator stats, definition versioning, exclusive branching gateways; parallel AND fork/join deferred). Phase 2 — Human Approval & SLA Governance 🔄 core complete: per-tenant `SlaPolicy` (SLA budget + ordered escalation chain), **multi-level chain-driven escalation** (breach → reassign up role → manager → executive with a fresh budget per level), task **reassignment/delegation**, and an `ApprovalNotificationPort` firing on raise + escalation — logging default plus config-gated best-effort **webhook** (`WebhookApprovalNotifier`, when `aether.flow.notification.webhook.url` is set) and **email** (`EmailApprovalNotifier` over `JavaMailSender`, when `aether.flow.notification.email.to` is set) sinks, both fanned in via `CompositeApprovalNotifier`, **operator lifecycle metrics** via `ApprovalMetricsPort` (Micrometer counters `aether.flow.approvals.{raised,approved,rejected,reassigned}`, alongside the sweep's escalation counter + open-queue gauge), and **per-tenant business-hours calendars** (`BusinessHours` on `SlaPolicy`, V006 — SLA budgets consume working time only; deadlines computed via `SlaPolicy.deadlineFrom` at both initial raise and escalation reset, 24/7 when a tenant has none); Testcontainers ITs. Phase 2 core complete.
...
| **SlaPolicy** | Per-tenant SLA budget + ordered escalation chain of roles + optional `BusinessHours` calendar (SLA budgets measured in working time; `deadlineFrom` is the single deadline computation used by raise + escalation). |
