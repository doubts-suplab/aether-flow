...
class ApprovalTaskControllerTest {
    ...
    void reassign_movesTaskToNewRole() {
        ...
        store.save(task);
        var controller = new ApprovalTaskController(store, new UnusedEngine());
    ...
    void reassign_missingRoleIsBadRequest() {
        ...
        var task = openTask("tenant-1");
        store.save(task);
        var res = new ApprovalTaskController(store, new UnusedEngine())
    ...
    void reassign_unknownTaskIsNotFound() {
        var res = new ApprovalTaskController(new FakeTaskStore(), new UnusedEngine())
